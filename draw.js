...
// TODO: ehh.... the map is 129x129 embark tiles, which is hard to convert into
// leaflet tiles. cheat for now.
const trueMapWidthInTiles = mapWidthInEmbarkTiles * 16
const mapWidthInTiles = (mapWidthInEmbarkTiles - 1)* 16
const mapHeightInTiles = (mapHeightInEmbarkTiles - 1)* 16
...
loadImage(fontFile).then(async (font) => {
  ...
  const maxZoom = Math.log2(Math.max(mapWidthInChunks, mapHeightInChunks))
  if (maxZoom !== Math.round(maxZoom)) throw new Error("don't yet know how to handle non-power-of-two map sizes")
  ...
  async function drawChunk(chunkX, chunkY) {
    ...
    for (let cy = 0; cy < chunkSizeInTiles; cy++) {
      for (let cx = 0; cx < chunkSizeInTiles; cx++) {
        ...
        const tile = map[absY*trueMapWidthInTiles+absX] || {c: 0, f: 0, b: 0, l: 0}
  ...
  async function drawZoomedOutChunk(z, x, y) {
    ...
    const [topLeft, topRight, bottomLeft, bottomRight] = await Promise.all([
      loadImage(chunkFileName(z + 1, x * 2, y * 2)),
      loadImage(chunkFileName(z + 1, x * 2 + 1, y * 2)),
      loadImage(chunkFileName(z + 1, x * 2, y * 2 + 1)),
      loadImage(chunkFileName(z + 1, x * 2 + 1, y * 2 + 1)),
    ])
    const canvas = createCanvas(chunkWidthInPixels, chunkHeightInPixels)
    const ctx = canvas.getContext('2d')
  ...
  for (let zoomLevel = maxZoom - 1; zoomLevel >= 0; zoomLevel--) {
    const chunksXInThisZoomLevel = mapWidthInChunks / Math.pow(2, maxZoom - zoomLevel)
    const chunksYInThisZoomLevel = mapHeightInChunks / Math.pow(2, maxZoom - zoomLevel)
